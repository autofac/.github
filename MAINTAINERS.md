# Autofac Maintainer Guide

Being an Autofac owner is a step beyond [being a contributor](CONTRIBUTING.md). Ownership is a longer-term commitment and, honestly, may not be for everyone. That's OK. If you don't think you're up for it, we would still love your help in the form of pull requests or answering questions on Stack Overflow.
...
There is an implicit trust between all of these projects and you. They trust that every change you make is doing something for the better. They trust that things have been well tested. They trust that if something has gone wrong in the latest release there's going to be someone - **you** - there to answer questions, help debug things, and correct issues.

They trust that [you're not doing anything malicious](https://snyk.io/blog/malicious-code-found-in-npm-package-event-stream/).
...
[Our code of conduct](CODE_OF_CONDUCT.md) applies to owners as much as anyone. Owners are responsible for enforcing it on each other and on contributors.
...
Owners have the final say about the shape of the public API. Do we need a new class here or a new interface there? Is that new method overload really helpful? As noted in the [contributor guide](CONTRIBUTING.md), there are a lot of things that may constitute breaking changes in the API. We can add things pretty easily, but removing them once the package has been taken is a major version increment and a breaking change for downstream consumers.
...
If the issue is a question, we generally try to get them to post to [Stack Overflow](https://stackoverflow.com/questions/tagged/autofac) and tag the question `autofac`. We do that because then the question can be searched by other people. People trying to figure out how something works largely won't come looking through the closed issues list in GitHub, so answering the question in GitHub only helps *that one person* while answering on Stack Overflow can help many people. We also discourage folks from double-posting questions - if it's on Stack Overflow *and* it's been filed on GitHub, put a link to the Stack Overflow question in the issue and close the issue. (This information should also be in the issue template in every Autofac repo, so folks double-posting aren't actually reading the issue template.)
...
- Does it adhere to our standards? The [contributor guide](CONTRIBUTING.md) outlines not only our general coding standards but the fact that everything should build and test with no errors and no warnings. Does the PR introduce any Roslyn analyzer warnings?
...
The channels we accept support requests through are listed in [SUPPORT.md](SUPPORT.md). At a minimum, as an owner you should subscribe to the `autofac` questions on Stack Overflow and to the discussion group.
...
Documentation is how many people understand how Autofac and its integrations work. It takes several forms:

- Long-form prose, like [the documentation site](https://docs.autofac.org/). Something that explains in detail how to use something, with examples.
- XML API documentation. These are the `<summary>` sorts of comments on members in the code. IntelliSense is generated from this content, as is the [HTML API documentation](https://autofac.org/apidoc/).
- Inline code comments. If there's a particularly difficult to understand section of code, or something that might be confusing ("Why did this variable have to get proactively disposed right here?"), a small comment explaining *why* helps other contributors avoid regressing defects or creating performance issues.

As an owner, you're responsible for *all of these types of documentation.* People around the world learn in different ways. Not all of them are ready to dive into GitHub repos and read the code. It's our job to accommodate the different levels of learners with the docs they require.

Examples may be:

- Small projects that demonstrate an end-to-end feature. We have a whole [Examples repository](https://github.com/autofac/Examples) for this. These are helpful to show - and test - real working integration features.
- Unit tests. These are harder to interpret for some users, but are good as "executable specifications" of how something should work.
...
The [contributor guide](CONTRIBUTING.md) talks about the different considerations. Briefly:

- Framework compatibility: raising the minimum target framework for a library means consumers below that target can no longer take an upgrade.
- API compatibility: adding a new member on an interface, moving an extension method to a different static class, or even adding an optional parameter on an existing method is a breaking public API change. Doing that breaks someone consuming the API if they take the upgrade.
- Dependency compatibility: if an Autofac component takes an upgrade to a dependency, it effectively forces all consumers of that component to take the upgrade too. In general we should leave that choice to the application developer where possible. There may be a technical reason to take an upgrade, like an integration that needs a new feature. Those choices need to be weighed individually.

As you maintain Autofac, it's up to you to determine when it's a good time to make a change that will potentially break the thousands of projects consuming the library. These are hard decisions. It's OK to ask for help.

From a general policy perspective, we should always be *compatible* with the latest release of a third-party framework we integrate with. That doesn't mean we *require* the latest version, only that we work with it. Where the two are mutually exclusive - we can *either* stay compatible with the older thing *or* move forward - we choose to move forward.

## Release Process

We follow the [Gitflow workflow process](https://www.atlassian.com/git/tutorials/comparing-workflows/gitflow-workflow/). Ongoing development happens on `develop`, and the release branch is `main`.

Pushes to `develop` publish packages to [GitHub Packages](https://github.com/orgs/autofac/packages), which is where you get builds for testing before a release. Pushing a `vX.Y.Z` tag publishes to both GitHub Packages and NuGet - **the tag push is what releases to NuGet, not the push of `main`.**
...
1. Check the semantic version is right for the changes since the last release, update it in `default.proj` if needed, and let a final CI build run on `develop`.
2. Fast-forward `main` up to `develop`:
...
3. Tag the release on `main` with `git tag vX.Y.Z`, push `main`, then push the tag.
4. Switch back to `develop` locally, so nothing lands on `main` by accident.
5. Confirm the two branches agree:
...
6. Write up the release on the GitHub releases page for the repository, explaining what changed. Packages come from the package sources; there is nothing to attach.
